...
def will_be(person)
    person_will_be = person + " will be " # parameter is kind of argument,
    return person_will_be
end


def wont_be(person)
    person_wont_be = person + " won't be"   #
    return person_wont_be
end

def impeached(string)
    new_string = string + "impeached"
    return new_string
end

def trump_predictor(method,person)
    return impeached(method(person)) #关于method能否作为parameter的问题
end

trump_predictor(will_be , "trump")
print : trump will be impeached

trump_predictor(wont_be)
...
def method(array,num)
    new_array = []
    array.each do |elem|        #在method里定义，就不用放在parameter里，任何变量在method外定义的，就一定要放在                                parameter里
      new_array  << num*elem
    end
     new_array
end
print method(array,10)
...
class Continent
    attr_accessor( :land_size, :hemisphere, :countries )
    def initialize(land_size, hemisphere, countries)
        @land_size = land_size
...
oceania = Continent.new(8.526,"south",countries)
puts oceania
puts oceania.countries

arr_states = ["Victoria", "New South Wale", "West Australia", "South Australia", "Northern Territory ", "Queensland","Tasmania"]
class country
    attr_accessor(:land_size, :population, :capital, :states)
        @land_size = land_size
...
class state
    attr_accessor(:land_size,:pupulation,:cities, :time_zone)
    @land_size = land_size
    @population = population
    @main_city = main_city
    @time_zone = time_zone
